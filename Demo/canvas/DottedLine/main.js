var Main = function () {
    ...
    //动画与连线
    var requestAnimFrame = requestAnimFrame || webkitRequestAnimationFrame || oRequestAnimationFrame || msRequestAnimationFrame;
    requestAnimFrame(animateUpdate);//兼容不同浏览器的requestAnimationFrame

    function animateUpdate(){
        ctx.clearRect(0,0, canvas.width, canvas.height);//清空canvas中原有内容
        ...
        for (var i = overNum; i < dotsNum; i++) {
            for(var j =i+1;j<dotsNum;j++){
                ...
                if(s<dotsDistance){
                    ...
                    ctx.strokeStyle ='rgba(225,225,225,' +(dotsDistance-s)/dotsDistance+')';
                    ctx.strokeWidth =1;
                    ctx.stroke();
                    ctx.closePath();
                }
